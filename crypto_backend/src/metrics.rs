...
impl MetricsCollector {
    ...
    pub fn generate_comparison_report(&self) -> String {
        ...
        let zkexp_summary = summaries.iter().find(|s| s.protocol_name.contains("RangeProof"));
        
        if let Some(zkexp) = zkexp_summary {
            report.push_str("\n--- RangeProof Performance Advantages ---\n");
            report.push_str(&format!("RangeProof baseline: {:.1}μs prove, {:.1}μs verify, {:.0}B proof\n",
