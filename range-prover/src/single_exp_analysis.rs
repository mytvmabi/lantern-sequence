...
impl SingleExponentiationAnalyzer {
    ...
    fn run_bit_range_tests(&mut self, bit_lengths: &[usize]) -> Result<(), String> {
        for &bit_length in bit_lengths {
            ...
            match self.system.prove_single_exponentiation(base, &exponent_bits) {
                Ok(proof) => {
                    ...
                    let verified = self.system.verify_single_exponentiation(&proof, expected);
                    let verify_time = verify_start.elapsed().as_micros() as u64;
                    
                    // Calculate efficiency score
                    let efficiency = if prove_time > 0 {
                        (bit_length as f64) / (prove_time as f64 / 1000.0)
                    } else {
                        0.0
                    };
